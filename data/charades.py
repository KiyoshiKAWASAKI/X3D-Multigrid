...
def make_dataset_old(split_file, split, root, num_classes=101):
    """
    split_file: annotation file
    """
    dataset = []
    # TODO: change the annotation file here
    data_txt = np.loadtxt(split_file, dtype="str")
    # TODO: change txt file into a dictionary
    print(data_txt)

    data = {}
    for i in range(len(data_txt[0])):
        data[i+1] = data_txt[i][1]
    ...
    if os.path.exists(pre_data_file):
        ...
        i = 0
        # Loop through each label in label set
...
def make_dataset(split_file, split, root, num_classes=101):
    ...
    output: data contains (vid, label, data[vid]['duration'], num_frames)
    vid: numerical label
    label: this should be the word label, names of the activities
    data[vid]['duration']: what is this, this doesnt seem to exit in UCF101??
    num_frames: calculate the number of frames we want
    """
    dataset = []
    data_txt = np.loadtxt(split_file, dtype="str")
